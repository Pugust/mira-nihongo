...
(() => {
  ...
  function selectObject(key,prediction,recognition){
    if(!JAPANESE_DB[key])return;const changed=state.selectedKey!==key;state.selectedKey=key;state.selectedPrediction=prediction||state.selectedPrediction;state.recognition={detectorScore:null,verifierScore:null,verifierLabel:'',family:null,...recognition};state.focus.trackingHash=(state.cameraStarted&&els.video.videoWidth)?computeCurrentFocusHash():null;state.focus.moving=0;setPhase('tracking');hideEmpty();els.lessonCard.classList.remove('hidden');if(changed){state.lessonPlan=null;state.contextState=null;state.photoStudyActive=false;if(state.sceneExplore){state.reviewActive=false;state.reviewRevealed=true;state.lastExposureSig='';registerHistory(key);state.actionIndex=0;state.revealPhase=1;setSheetSnap('compact',false);clearTimeout(state.revealTimer);clearTimeout(state.reviewTimer)}else{registerEncounter(key);registerHistory(key);state.actionIndex=0;state.revealPhase=0;setSheetSnap('compact',false);clearTimeout(state.revealTimer);clearTimeout(state.reviewTimer);if(state.reviewActive){state.reviewTimer=setTimeout(()=>revealReviewNow(false),1800)}else{state.revealTimer=setTimeout(()=>{state.revealPhase=1;renderLesson()},650)}}}renderLesson();setStatus(state.recognition.kind==='tentative'?'Provavelmente · confirme':'Reconhecido');haptic(14);if(state.autoFreezeEnabled&&state.cameraStarted&&!state.frozen)setFrozen(true,'recognition');if(state.tutorial.active&&state.tutorial.step===0)showTutorialStep(1);
  }
  ...
  function revealReviewNow(notify=true){if(!state.reviewActive&&!state.reviewRevealed)return;clearTimeout(state.reviewTimer);state.reviewRevealed=true;state.reviewActive=false;state.revealPhase=1;renderLesson();if(notify)showToast('Resposta revelada. Marque “Já sei” ou “Quero revisar” se quiser ajustar o ritmo.')}
